...
// DB 是一个数据库句柄，表示一个包含零个或多个底层连接的池。
//多个 goroutine 并发使用它是安全的。
//sql 包自动创建和释放连接；
//如果数据库具有每个连接状态的概念，则可以在事务 (Tx) 或连接 (Conn) 中可靠地观察到此类状态。
//调用 DB.Begin 后，返回的 Tx 将绑定到单个连接。 一旦在事务上调用 Commit 或 Rollback，该事务的连接将返回到 DB 的空闲连接池。
//可以使用 SetMaxIdleConns 控制池大小。
type DB struct {
